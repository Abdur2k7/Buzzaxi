...
var init_assets = __esm({
  "embedded:assets"() {
    ...
    surface = "https://cdn.jsdelivr.net/gh/fadeichev2121/planet@b3f70fbf4b577845b1d9d5947c9410fb4d925dae/models/surface.json";
  }
});
...
function usePlanetAsset(onReady) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchChecked(surface).catch(() => fetchChecked(`${base}models/surface.json`)).then((response) => response.json()).catch(() => ({ width: 64, height: 32, radii: new Array(2048).fill(0.95) }))
...
function surfaceRadius(surface2, x, y, z) {
  if (!surface2 || !surface2.radii) return 0.95;
...
var init_bagSuspension = __esm({
  "src/bagSuspension.ts"() {
    ...
    BagSuspension = class {
      ...
      rollVelocity = 0;
      restore() {
        if (!this.applied || !this.bone) return;
      ...
      update(dt, activity, _phase, turnRate, reduced, bodyLean = 0) {
        if (!this.bone || dt <= 0 || !this.bone.parent) return;
        ...
        const targetPitch = reduced ? 0 : MathUtils3.clamp(Math.atan2(this.acceleration.z, gravity), -0.1, 0.1);
        const targetRoll = reduced ? 0 : MathUtils3.clamp(Math.atan2(-this.acceleration.x, gravity) + Math.abs(turnRate) * 1e-3, -0.012, 0.045);
    ...
    CourierGreeting = class {
      constructor(scene) {
        ...
        this.joints = ["RightArm", "RightForeArm", "RightHand"].map((name) => {
          ...
          if (!(bone instanceof Bone2)) return null;
      ...
      apply(reduced) {
        if (this.weight === 0 || this.joints.length < 3) return;
...
var SceneBoundary = class extends Component {
  ...
  render() {
    return this.state.failed ? <div className="scene-fallback"><p>We could not load this 3D scene.</p><button onClick={() => location.reload()}>Try again</button></div> : this.props.children;
  }
};
var stories = {
  "How it works": { title: "From pickup to destination.", paragraphs: ["Every delivery begins with a booking. Set your pickup point, choose your destination, and confirm your shuttle.", "Our shared fleet keeps vehicles moving efficiently across high-demand city corridors."] },
  "For business": { title: "Fleet transit for your team.", paragraphs: ["Daily employee commutes and corporate transfers on scheduled routes.", "Buzzaxi provides fixed pricing and verified drivers for predictable city travel."] },
  "Our story": { title: "Clear pricing. Reliable routes.", paragraphs: ["Built to solve everyday traffic congestion in Mumbai.", "Buzzaxi connects riders headed along the same routes to lower costs and reduce road density."] }
};
function App() {
  ...
  return <div className={`page ${prototype ? "prototype" : ""}`}>
    <header className="site-header">
      <a href="./" className="wordmark" aria-label="Buzzaxi home"><span className="brand-type">buzzaxi<small>transit network</small></span></a>
      <nav aria-label="Main navigation"><button onClick={explore}>Routes</button>{["How it works", "For business", "Our story"].map((item) => <button key={item} onClick={() => setStory(item)}>{item}</button>)}</nav>
      <button className="header-cta" onClick={() => {
    ...
    <main><section className="hero" aria-labelledby="hero-title">
      <div className="hero-copy">
        <p className="eyebrow">Mumbai Shared Transit</p>
        <h1 id="hero-title">Shared cabs.<br />Fixed rates.<br /><em>Zero surges.</em></h1>
        <p className="hero-description">Book shared autos, sedans, and SUVs across Mumbai corridors. Clear upfront pricing with verified drivers.</p>
        <button className="explore-button" onClick={() => {
          const bookingElem = document.getElementById('booking-section');
          if (bookingElem) bookingElem.scrollIntoView({ behavior: 'smooth' });
        }}>Choose Route <Arrow /></button>
      </div>
      <div className="visual-column">
        ...
        >
          ...
          {!ready && <div className="loading" role="status"><span />Loading interactive 3D view…</div>}
        </div>
      </div>
      <div className={`planet-caption ${dragging ? "is-dragging" : ""}`} aria-hidden="true"><p>{!auto ? "Press Start" : dragging ? "Rotating" : "Drag to rotate"}<br />{!auto ? "to resume" : dragging ? "city map" : "the 3D route"}</p><svg viewBox="0 0 180 165" fill="none"><path d="M161 148C137 82 103 39 28 14m0 0 6 16m-6-16 19-2" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" /></svg></div>
      <p id="planet-instructions" className="sr-only">Drag to rotate the 3D globe. Space pauses rotation.</p>
    </section></main>
    <footer className="site-footer"><p className="footer-left">Shared Cabs & Autos<br />Mumbai, Maharashtra</p><button className="motion-button" onClick={toggleMotion} aria-pressed={!auto} aria-label={auto ? "Pause" : "Start"}>{auto ? <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M7 5v10m6-10v10" stroke="currentColor" strokeWidth="1.5" /></svg> : <svg viewBox="0 0 20 20" aria-hidden="true"><path d="m7 4 8 6-8 6Z" fill="currentColor" /></svg>}<span>{auto ? "Pause" : "Start"}</span></button><p className="footer-right">18 Fixed Routes<br />24/7 Service</p></footer>
...
function StoryDialog({ story, onClose }) {
  ...
  }}><button className="close-dialog" aria-label="Close" onClick={onClose}>×</button><span className="eyebrow">{story}</span><h2>{stories[story].title}</h2>{stories[story].paragraphs.map((p) => <p key={p}>{p}</p>)}<button className="explore-button" onClick={onClose}>Close</button></dialog>;
}

// Clean flat style without blurry glassmorphism or purple gradients
var css = `
.orbit-delivery{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#111827;background:#ffffff;font-synthesis:none;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;font-weight:400}
.orbit-delivery *{box-sizing:border-box}.orbit-delivery{margin:0}.orbit-delivery button,.orbit-delivery a{-webkit-tap-highlight-color:transparent}.orbit-delivery button{font:inherit;color:inherit;cursor:pointer;border:0;background:none}.orbit-delivery button:disabled{cursor:default;opacity:.55}.orbit-delivery button:focus-visible,.orbit-delivery a:focus-visible{outline:2px solid #111827;outline-offset:3px}.orbit-delivery a{color:inherit;text-decoration:none}.orbit-delivery svg{display:block}.orbit-delivery button svg{width:20px;height:20px}
.orbit-delivery .page{height:720px;min-height:600px;position:relative;overflow:hidden;background:#f9fafb;border-bottom:1px solid #e5e7eb;display:flex;flex-direction:column}
.orbit-delivery .site-header{height:80px;flex:none;display:flex;align-items:center;justify-content:space-between;padding:0 5%;position:relative;z-index:5}
.orbit-delivery .wordmark{font-size:24px;font-weight:700;letter-spacing:-0.5px;color:#111827}
.orbit-delivery .brand-type{display:flex;flex-direction:column;line-height:1;gap:3px}
.orbit-delivery .brand-type small{font-size:10px;font-weight:600;letter-spacing:.15em;text-transform:uppercase;color:#d97706}
.orbit-delivery .site-header nav{position:absolute;left:50%;transform:translateX(-50%);display:flex;gap:36px;align-items:center}
.orbit-delivery .site-header nav button{font-size:14px;color:#4b5563;font-weight:500;transition:color .15s}
.orbit-delivery .site-header nav button:hover{color:#111827}
.orbit-delivery .header-cta{background:#111827;color:#ffffff;border-radius:6px;padding:10px 20px;font-size:14px;font-weight:600}
.orbit-delivery .header-cta:hover,.orbit-delivery .explore-button:hover{background:#1f2937}
.orbit-delivery main{flex:1;min-height:0;display:flex}
.orbit-delivery .hero{width:100%;position:relative}
.orbit-delivery .hero-copy{position:relative;z-index:3;margin-left:5%;padding-top:70px;width:44%;pointer-events:none}
.orbit-delivery .hero-copy button{pointer-events:auto}
.orbit-delivery .eyebrow{text-transform:uppercase;letter-spacing:.15em;font-size:12px;font-weight:700;color:#d97706;margin:0 0 16px}
.orbit-delivery h1{font-size:54px;font-weight:800;letter-spacing:-1.5px;line-height:1.1;color:#111827;margin:0 0 18px}
.orbit-delivery h1 em{font-style:normal;color:#d97706}
.orbit-delivery .hero-description{color:#4b5563;font-size:17px;line-height:1.5;margin:0 0 26px}
.orbit-delivery .explore-button{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:12px 24px;border-radius:6px;background:#d97706;color:#ffffff;font-size:15px;font-weight:600}
.orbit-delivery .explore-button:hover{background:#b45309}
.orbit-delivery .visual-column{position:absolute;right:0;top:0;width:60%;height:100%;z-index:1}
...
.orbit-delivery .planet-caption{position:absolute;right:40px;top:100px;z-index:3;width:140px;pointer-events:none;color:#6b7280}
.orbit-delivery .planet-caption p{font-size:13px;line-height:1.3;text-align:right;margin:0;font-weight:500}
.orbit-delivery .planet-caption svg{width:120px;height:100px;margin-top:-10px;margin-left:-20px}
.orbit-delivery .site-footer{position:absolute;bottom:24px;left:5%;right:5%;z-index:4;display:flex;align-items:flex-end;justify-content:space-between;pointer-events:none}
.orbit-delivery .site-footer p{margin:0;text-transform:uppercase;font-size:10px;font-weight:600;letter-spacing:.12em;line-height:1.6;color:#6b7280}
.orbit-delivery .motion-button{display:flex;gap:6px;align-items:center;color:#4b5563;pointer-events:auto;font-size:11px;font-weight:600;padding:6px 12px;background:#ffffff;border:1px solid #e5e7eb;border-radius:4px}
.orbit-delivery .motion-button:hover{background:#f3f4f6}
.orbit-delivery .loading{position:absolute;top:45%;left:25%;right:20%;display:flex;align-items:center;justify-content:center;gap:10px;color:#6b7280;font-size:13px;font-weight:500}
.orbit-delivery .loading>span{width:16px;height:16px;border:2px solid #e5e7eb;border-top-color:#111827;border-radius:50%;animation:loading 1s linear infinite}
@keyframes loading{to{transform:rotate(360deg)}}
.orbit-delivery .scene-fallback{position:absolute;inset:35% 20%;font-size:14px;text-align:center;color:#6b7280;z-index:5}
.orbit-delivery .scene-fallback button{background:#111827;border-radius:4px;color:#fff;padding:8px 16px;margin-top:10px}
.orbit-delivery .about-dialog{border:1px solid #e5e7eb;border-radius:8px;padding:36px;max-width:480px;width:calc(100% - 32px);background:#ffffff;color:#111827;box-shadow:0 10px 25px rgba(0,0,0,0.1)}
.orbit-delivery .about-dialog::backdrop{background:rgba(0,0,0,0.4)}
.orbit-delivery .about-dialog h2{font-size:26px;font-weight:700;letter-spacing:-0.5px;margin:16px 0 14px}
.orbit-delivery .about-dialog p{font-size:14px;line-height:1.6;color:#4b5563}
.orbit-delivery .about-dialog .explore-button{margin-top:14px;font-size:13px}
.orbit-delivery .close-dialog{position:absolute;right:16px;top:12px;font-size:24px;color:#9ca3af}
...
@media(max-width:768px){
  .orbit-delivery .page{height:auto;min-height:580px}
  .orbit-delivery .site-header nav{display:none}
  .orbit-delivery .hero-copy{width:calc(100% - 40px);margin:0 20px;padding-top:24px}
  .orbit-delivery h1{font-size:38px}
  .orbit-delivery .visual-column{position:relative;width:100%;height:380px}
  .orbit-delivery .planet-caption{display:none}
}
`;

function OrbitDeliveryHero({ theme = "light", assetBaseUrl = "https://cdn.jsdelivr.net/gh/fadeichev2121/planet@b3f70fbf4b577845b1d9d5947c9410fb4d925dae" }) {
